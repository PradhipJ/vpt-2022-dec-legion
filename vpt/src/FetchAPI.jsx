...
async function getThumbnailURL(isbn, callback) {
  ...
  fetch(
    ...
    .then(() => callback(thumbnail_url));
  return thumbnail_url;
}

function getValidImage(isbnArrays) {
  validURL = "";
  for (i = 0; i < isbnArrays.length; i++) {
    validURL = getThumbnailURL(isbnArrays[i]);
...
function getSearchResultByBookAndAuthor(book, author, page = 1) {
  fetch(
    `https://openlibrary.org/search.json?q=${book.replace(
      /\s+/g,
      "+"
